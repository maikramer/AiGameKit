"""Kill GPU respeita a fila vramd — não mata quando há jobs."""
...
class TestKillRespectsVramdQueue:
    def test_refuses_when_vramd_busy(self) -> None:
        snap = {
            "inflight": 1,
            "queue_depth": 0,
        ...
        with (
            ...
            patch("aigamekit_shared.vramd_client.fetch_vramd_queue_snapshot", return_value=snap),
        ...
        assert any("vramd" in line for line in logs)

    def test_proceeds_when_ums_idle(self) -> None:
        with (
            patch("aigamekit_shared.vramd_client.is_vramd_running", return_value=True),
            patch(
                "aigamekit_shared.vramd_client.fetch_vramd_queue_snapshot",
