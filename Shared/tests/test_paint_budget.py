...
class TestPaintTargetFaces:
    ...
    def test_1024_in_band(self) -> None:
        ...
        assert f == 115_343  # 1024^2 * 0.55 / 5

    def test_min_floor(self) -> None:
        assert PAINT_FACES_MIN == 6_000
        # O piso deixa de vincular no ladder atual (256 → ~7.2k raw).
        assert paint_target_faces(256) == 7_208  # 256^2 * 0.55 / 5
    ...
    def test_chapel_clean_starved(self) -> None:
        # 2.27M faces @ 2k ≈ 0.5 texel/tri — orçamento correcto ~7 tpt no cap
        assert texels_per_face(2048, 2_270_000) < 2.0
        assert texels_per_face(2048, paint_target_faces(2048)) >= 5.0


class TestPaintTextureForChar:
    def test_bucket_vs_house_anchor(self) -> None:
        """Balde ~512/~29k faces; casa medium 2048/320k; highest casa 4096."""
