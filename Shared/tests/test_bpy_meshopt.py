...
def test_gltf_meshopt_export_kwargs_empty_when_unsupported() -> None:
    with patch("aigamekit_shared.bpy_mesh.gltf_export_supports_meshopt", return_value=False):
        assert gltf_meshopt_export_kwargs(enable=True) == {}


def test_gltf_meshopt_export_kwargs_when_supported() -> None:
    with patch("aigamekit_shared.bpy_mesh.gltf_export_supports_meshopt", return_value=True):
...
def test_gltf_import_supports_meshopt_version_gate() -> None:
    with patch("aigamekit_shared.bpy_mesh.bpy_version_tuple", return_value=(5, 1, 2)):
        assert gltf_import_supports_meshopt() is False
    with patch("aigamekit_shared.bpy_mesh.bpy_version_tuple", return_value=(5, 2, 0)):
        assert gltf_import_supports_meshopt() is True
    with patch("aigamekit_shared.bpy_mesh.bpy_version_tuple", return_value=None):
        assert gltf_import_supports_meshopt() is False
