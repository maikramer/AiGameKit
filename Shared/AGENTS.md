...
| `gltf_decode.py` | ~200 | bpy_mesh, GameDevLab, Animator3D | `bpy_readable_glb()` — KTX2/`ktxdecompress`, meshopt-on-old-bpy via `@gltf-transform/cli` |
...
| `env.py` | 133 | 10+ files | `TOOL_BINS`, `prefer_monorepo_tools` (`GAMEDEV_PREFER_MONOREPO`), `subprocess_gpu_env` |
...
1. **GPU/VRAM** (`gpu.py` + `vram_monitor.py` + `cli_helpers.py`): NVML-first inventory; UMS delegation before prep; `prepare_gpu_exclusive` only in-process fallback; kill respects UMS queue; legacy servers need `GAMEDEV_ALLOW_LEGACY_SERVER=1`.
...
6. **File logging** (`logging.py`): process-wide sink → `~/.cache/gamedev/logs/<tool>-YYYY-MM-DD.log`. `configure_logging(tool)` + `Logger`; stdlib root bridged once. Off under pytest unless `GAMEDEV_FILE_LOG=1`. Docs: `docs/LOGGING.md`.
...
Kill GPU tests: `patch(gamedev_shared.gpu.os.kill)` = patch **global** `os.kill`
