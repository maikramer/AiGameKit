...
O cache fica **fora do git**, em ``$XDG_CACHE_HOME/gamedev/quaternius/`` (fallback
...
Env vars:
    ...
    GAMEDEV_CACHE_DIR: diretório base de cache (default: XDG/``~/.cache/gamedev``).
"""
...
def cache_dir() -> Path:
    """Diretório base de cache do GameDev (fora do git).

    Ordem de precedência: ``GAMEDEV_CACHE_DIR`` → ``XDG_CACHE_HOME/gamedev`` →
    ``~/.cache/gamedev``.
    """
    env = os.environ.get("GAMEDEV_CACHE_DIR")
    if env:
        return Path(env)
