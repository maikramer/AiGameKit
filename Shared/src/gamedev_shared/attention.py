...
Env vars:
  - ``GAMEDEV_ATTENTION_BACKEND``: ``"auto"`` (default) | ``"sage"`` | ``"flash"`` | ``"sdpa"``
"""
...
def get_attention_backend() -> str:
    """Backend de attention preferido (env ``GAMEDEV_ATTENTION_BACKEND``).

    Auto-detect: sage → flash → sdpa (default seguro).
    """
    requested = os.environ.get("GAMEDEV_ATTENTION_BACKEND", "auto").strip().lower()
    if requested == "auto":
