...
Env:
    ``GAMEDEV_GLB_VERIFY`` — ``0`` desliga; default ligado.
    ``GAMEDEV_GLB_VERIFY_STRICT`` — ``1`` levanta ``GlbVerifyError`` em fail.
    ``GAMEDEV_GLB_VERIFY_MAX_VPT`` — teto V/Tri (default ``1.6``).
"""
...
def verify_glb(
    ...
    *,
    ...
    if max_v_per_tri is None:
        try:
            max_v_per_tri = float(os.environ.get("GAMEDEV_GLB_VERIFY_MAX_VPT", _DEFAULT_MAX_VPT))
        except ValueError:
...
def post_save_verify(
    ...
    *,
    ...
    """
    if not _truthy("GAMEDEV_GLB_VERIFY", default=True):
        return None
    ...
    if not result.ok and _truthy("GAMEDEV_GLB_VERIFY_STRICT", default=False):
