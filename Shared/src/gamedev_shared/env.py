...
GAMEDEV_PREFER_MONOREPO = "GAMEDEV_PREFER_MONOREPO"
...
TOOL_LAYOUT: dict[str, ToolLayout] = {
    ...
    "gamedevlab": ToolLayout("GameDevLab", "gamedev-lab"),
    ...
    "modelserver": ToolLayout("ModelServer", "gamedev-model-server"),
...
def prefer_monorepo_tools() -> bool:
    ...
    Default ``True``. Desliga com ``GAMEDEV_PREFER_MONOREPO=0`` (instalação só via
    ``~/.local/bin`` / catálogo remoto sem checkout).
    """
    raw = os.environ.get(GAMEDEV_PREFER_MONOREPO, "1").strip().lower()
...
def discover_monorepo_tool_bin(
    ...
    *,
    ...
    Args:
        tool_name: Chave em :data:`TOOL_LAYOUT` (ex. ``text3d``, ``gamedevlab``).
...
def apply_monorepo_tool_bins(env: dict[str, str]) -> dict[str, str]:
    ...
    Não sobrescreve variáveis já definidas. No-op se ``GAMEDEV_PREFER_MONOREPO=0``
