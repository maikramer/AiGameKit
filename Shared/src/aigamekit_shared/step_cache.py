...
Env vars:
  - ``AIGAMEKIT_STEP_CACHE``: ``"auto"`` (default) | ``"first_block"`` | ``"taylorseer"`` | ``"off"``
"""
...
def get_step_cache_mode() -> str:
    """Modo de step cache activo (env ``AIGAMEKIT_STEP_CACHE``)."""
    return os.environ.get("AIGAMEKIT_STEP_CACHE", "off").strip().lower()
