...
def server_socket_path(tool: str) -> Path:
    ...
    override = os.environ.get("AIGAMEKIT_MODEL_SERVER_SOCKET", "").strip()
    if override:
...
def resolve_ums_priority(explicit: str | None = None) -> str:
    """Resolve prioridade UMS: argumento → env ``AIGAMEKIT_UMS_PRIORITY`` → interactive."""
    if explicit:
        return str(explicit).strip().lower()
    env = os.environ.get("AIGAMEKIT_UMS_PRIORITY", "").strip().lower()
...
UMS_DO_NOT_KILL_TIP = (
    "VRAM ocupada? Usa `aigamekit-model-server status` / `queue` — espera ou "
    "`aigamekit-model-server cancel <job_id>`. NÃO mates processos GPU "
...
def _discover_modelserver_python() -> Path | None:
    ...
    # Shared/src/aigamekit_shared/model_server.py → monorepo root = parents[3]
...
def _resolve_ums_start_cmd(
    *,
    ...
    Prioridade (supervisor nunca deve herdar o venv da tool que o chama):
      ...
      3. ``aigamekit-model-server`` / ``ums`` no PATH (instalados pelo install.sh)
    ...
    on_path = lookup("aigamekit-model-server") or lookup("ums")
    if on_path:
...
def ensure_ums_running(*, timeout_sec: float = 30.0, auto_start: bool = True) -> bool:
    ...
    ``aigamekit-model-server start`` e espera até o socket ficar pronto.

    O auto-start pode ser desativado com a env var ``AIGAMEKIT_UMS_AUTO_START=0``.
    ...
    if os.environ.get("AIGAMEKIT_UMS_AUTO_START", "1") == "0":
        return False
    ...
    log_path = Path(
        os.environ.get("AIGAMEKIT_UMS_AUTO_START_LOG", "").strip()
        or (Path.home() / ".cache" / "aigamekit" / "ums-autostart.log")
    )
    try:
...
def delegate_to_ums(
    ...
    *,
    ...
    em background (a menos que ``AIGAMEKIT_UMS_AUTO_START=0``).

    Prioridade: ``priority`` explícito, senão env ``AIGAMEKIT_UMS_PRIORITY``,
    senão ``interactive``. GameAssets batch deve exportar ``AIGAMEKIT_UMS_PRIORITY=batch``.
...
def ensure_vram_available(
    ...
    *,
    ...
    allow_legacy = os.environ.get("AIGAMEKIT_ALLOW_LEGACY_SERVER", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
    ...
    if not allow_legacy:
        ...
        _logger.warn(
            ...
            "ensure_vram desligado — arranca UMS ou AIGAMEKIT_ALLOW_LEGACY_SERVER=1"
        )
        return False
