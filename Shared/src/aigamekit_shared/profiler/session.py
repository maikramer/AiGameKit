...
class ProfilerSession:
    ...
    Quando ``AIGAMEKIT_PERF_DB`` está definido ou o módulo
    :mod:`aigamekit_shared.perfstore` está disponível, os spans também são
    ...
    def _init_perf_recorder(self) -> None:
        if os.environ.get("AIGAMEKIT_PERF_DB", "").strip() == "off":
            return
        try:
    ...
    def span(self, name: str, *, sync_cuda: bool = False) -> Iterator[None]:
        ...
        try:
            ...
            if self._perf_recorder is not None:
                try:
                    from aigamekit_shared.perfstore.models import SpanRecord
