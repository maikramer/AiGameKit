...
Env:
    ``AIGAMEKIT_PIPELINE_TRACE`` — ``0`` desliga; default ligado.
    ``AIGAMEKIT_PIPELINE_TRACE_LOG`` — caminho do JSONL (default
    ``~/.cache/aigamekit/logs/pipeline-trace-YYYY-MM-DD.jsonl``).
    ``AIGAMEKIT_PIPELINE_TRACE_ASSET`` — se definido, só regista eventos cujo
...
def enabled() -> bool:
    return _truthy("AIGAMEKIT_PIPELINE_TRACE", default=True)
...
def default_log_path() -> Path:
    override = os.environ.get("AIGAMEKIT_PIPELINE_TRACE_LOG", "").strip()
    if override:
        return Path(override).expanduser().resolve()
    base = os.environ.get("AIGAMEKIT_LOG_DIR", "").strip()
    root = Path(base).expanduser() if base else Path.home() / ".cache" / "aigamekit" / "logs"
...
def trace_event(event: str, **fields: Any) -> Path | None:
    ...
    asset_filter = os.environ.get("AIGAMEKIT_PIPELINE_TRACE_ASSET", "").strip().lower()
    if asset_filter:
