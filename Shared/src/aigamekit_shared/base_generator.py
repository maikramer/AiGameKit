...
class DiffusionGeneratorBase(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        # None = respeitar env AIGAMEKIT_TORCH_COMPILE; True/False = override CLI.
        self.torch_compile = torch_compile
        self.torch_compile_mode = torch_compile_mode
        # None = respeitar env AIGAMEKIT_STEP_CACHE; str = "off"|"auto"|"first_block"|"taylorseer".
    ...
    def _try_group_offload(self, pipe: Any, *, config: Any | None = None) -> bool:
        ...
        ``AIGAMEKIT_GROUP_OFFLOAD`` (default habilitado).

        Args:
            pipe: pipeline diffusers.
            config: :class:`~aigamekit_shared.group_offload.GroupOffloadConfig`
    ...
    def _place_with_planner(
        ...
        *,
        ...
        :func:`~aigamekit_shared.lowvram.place_pipeline` — que decide multi-GPU vs
        ...
        Args:
            pipe: pipeline diffusers.
            footprint: :class:`~aigamekit_shared.lowvram.ModelFootprint`. Se tem
        ...
        Returns:
            :class:`~aigamekit_shared.lowvram.OffloadPlan` resolvido e aplicado.
        """
    ...
    def _torch_compile_enabled(self) -> bool:
        """Resolve se compile está activo: instance flag > env ``AIGAMEKIT_TORCH_COMPILE``."""
        import os

        if self.torch_compile is not None:
            return bool(self.torch_compile)
        return os.environ.get("AIGAMEKIT_TORCH_COMPILE", "0").strip().lower() in ("1", "true", "yes", "on")

    def _maybe_compile_transformer(self, pipe: Any, plan: Any) -> None:
        """Compila o transformer via torch.compile quando seguro.

        **Opt-in** via ``--compile`` / ``AIGAMEKIT_TORCH_COMPILE=1``: cold-start do
    ...
    def _maybe_apply_step_cache(self, pipe: Any, plan: Any) -> None:
        ...
        step caching é speed, não VRAM. CLI ``--step-cache`` > env ``AIGAMEKIT_STEP_CACHE``.
        """
    ...
    def _maybe_select_attention_backend(self, pipe: Any, plan: Any) -> None:
        ...
        Controlado por ``AIGAMEKIT_ATTENTION_BACKEND`` env (default "auto").
        """
        if self.device == "cpu":
            return
