"""Logger unificado Rich/ANSI + ficheiro para o monorepo AiGameKit.

Por omissão grava em ``~/.cache/aigamekit/logs/<tool>-YYYY-MM-DD.log``
(ou ``$AIGAMEKIT_LOG_DIR`` / ``$AIGAMEKIT_LOG_FILE``). Desligar com
``AIGAMEKIT_FILE_LOG=0`` ou ``AIGAMEKIT_NO_FILE_LOG=1``.

Em pytest o ficheiro fica desligado salvo ``AIGAMEKIT_FILE_LOG=1``.
"""
...
def detect_tool_name() -> str:
    """Nome da tool para o ficheiro de log (env → argv → ``aigamekit``)."""
    env = os.environ.get("AIGAMEKIT_LOG_TOOL", "").strip()
    if env:
        return env
    prog = Path(sys.argv[0]).name if sys.argv else "aigamekit"
    return _TOOL_ALIASES.get(prog, prog or "aigamekit")


def file_logging_enabled() -> bool:
    """True se logging para ficheiro está ativo."""
    explicit = os.environ.get("AIGAMEKIT_FILE_LOG", "").strip().lower()
    if explicit in ("0", "false", "no", "off"):
        return False
    no = os.environ.get("AIGAMEKIT_NO_FILE_LOG", "").strip().lower()
    if no in ("1", "true", "yes", "on"):
        return False
    # Pytest: off por omissão (evita poluir ~/.cache); on com AIGAMEKIT_FILE_LOG=1.
...
def default_log_dir() -> Path:
    """Diretório de logs: ``AIGAMEKIT_LOG_DIR`` ou ``<cache>/logs``."""
    env = os.environ.get("AIGAMEKIT_LOG_DIR", "").strip()
    if env:
        return Path(env)
...
def resolve_log_path(tool: str | None = None) -> Path:
    """Path do ficheiro de log para a tool (ou ``AIGAMEKIT_LOG_FILE``)."""
    override = os.environ.get("AIGAMEKIT_LOG_FILE", "").strip()
    if override:
...
def _parse_min_level() -> int:
    raw = os.environ.get("AIGAMEKIT_LOG_LEVEL", "INFO").strip().upper() or "INFO"
...
def configure_logging(
    ...
    *,
    ...
    Args:
        ...
        log_dir: Override de ``AIGAMEKIT_LOG_DIR`` para este processo.
        log_file: Override de ``AIGAMEKIT_LOG_FILE`` (path exacto).
    ...
    if tool:
        os.environ["AIGAMEKIT_LOG_TOOL"] = tool
        _tool_name = tool
    if log_dir is not None:
        os.environ["AIGAMEKIT_LOG_DIR"] = str(log_dir)
    if log_file is not None:
        os.environ["AIGAMEKIT_LOG_FILE"] = str(log_file)
    if level is not None:
        os.environ["AIGAMEKIT_LOG_LEVEL"] = level.upper()
        _min_rank = _parse_min_level()
...
class _StdlibFileHandler(_stdlib_logging.Handler):
    """Handler stdlib que reutiliza o sink de ficheiro do AiGameKit."""
...
def bridge_stdlib_logging(*, level: int | None = None) -> None:
    """Liga o root logger stdlib ao ficheiro AiGameKit (uma vez por processo)."""
