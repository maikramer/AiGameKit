...
class FastGeometryExtractorV2(BaseGeometryExtractor):
    ...
    def extract_geometry(
        ...
        bounds: tuple[float] | list[float] | float = (-1.25, -1.25, -1.25, 1.25, 1.25, 1.25),
        octree_depth: int = 7,
        ...
        **kwargs,
        ...
        # CPU: os chunks vão para a GPU via prefetch stream (iter_query_chunks)
        # — evita alojar o grid inteiro na VRAM antes do decode.
        ...
        batch_logits = []
        for queries in self.iter_query_chunks(
            xyz_samples,
            num_chunks,
            disable_tqdm=disable_tqdm,
            desc=f"MC Level {mc_level} Implicit Function:",
        ):
            batch_queries = repeat(queries.half(), "p c -> b p c", b=1)
    ...
    def extract_geometry_with_stats(self, geometric_func: Callable, **kwargs) -> tuple[trimesh.Trimesh, dict]:
        """
        提取几何并返回统计信息
        """
...
def extract_geometry_fast_v2(device: torch.device = None, *args, **kwargs):
    ...
    return ([(mesh.vertices, mesh.faces)] if len(mesh.vertices) > 0 else [(None, None)]), np.array(
        [len(mesh.vertices) > 0], dtype=np.bool_
    )
