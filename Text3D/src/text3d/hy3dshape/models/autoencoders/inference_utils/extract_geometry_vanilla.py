    ...
    def extract_geometry(
        ...
        try:
            if mc_mode == "mc":
                ...
                # Grid-space → bounds por-eixo (o antigo `*2` assumia cubo ±1 e
                # distorcia bounds anisotrópicos).
                size = torch.tensor(bbox_size, device=verts.device, dtype=verts.dtype)
                verts = self.center_vertices(verts) * size
