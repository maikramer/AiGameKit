...
class BaseGeometryExtractor(ABC):
    ...
    def iter_query_chunks(
        ...
        *,
        ...
        desc: str = "Implicit Function:",
    ):
        """Itera chunks de queries com prefetch CPU→GPU em stream dedicada.

        Double-buffer: enquanto o geo-decoder computa o chunk N, a copy-stream
        já transfere o N+1 (pinned + non_blocking) — mesmo padrão do group
        offload com streams. Sem CUDA (ou samples já na GPU) degrada para o
        loop síncrono clássico.
        """
        from tqdm import tqdm as _tqdm

        total = int(xyz_samples.shape[0])
        starts = list(range(0, total, int(num_chunks)))
        device = self.device if isinstance(self.device, torch.device) else torch.device(self.device)
        use_stream = (
            device.type == "cuda" and torch.cuda.is_available() and xyz_samples.device.type == "cpu" and len(starts) > 1
        )

        if not use_stream:
            for start in _tqdm(starts, desc=desc, disable=disable_tqdm, leave=False):
                yield xyz_samples[start : start + num_chunks, :].to(device)
            return

        copy_stream = torch.cuda.Stream(device=device)

        def _stage(start: int):
            pinned = xyz_samples[start : start + num_chunks, :].pin_memory()
            ev = torch.cuda.Event()
        ...
        nxt = _stage(starts[0])
        for i in _tqdm(range(len(starts)), desc=desc, disable=disable_tqdm, leave=False):
            cur, ev = nxt
            if i + 1 < len(starts):
                nxt = _stage(starts[i + 1])
            # Espera SÓ a cópia do chunk atual — a do próximo continua em
            # paralelo com o compute deste.
            torch.cuda.current_stream(device).wait_event(ev)
            cur.record_stream(torch.cuda.current_stream(device))
            yield cur
    ...
    def evaluate_sdf_batch(self, sdf_func: Callable, points: torch.Tensor, batch_size: int = 100000) -> torch.Tensor:
        """
        分批评估SDF函数，避免内存溢出
