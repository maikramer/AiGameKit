"""
Decimação UV-aware e remesh voxel de GLB texturado (bpy backend).

``remesh_textured_glb``: Decimate COLLAPSE com reparo prévio
(``pre_decimate_uv``) e reprojeção/preservação de materiais.

``remesh_glb`` (comando ``remesh``): voxel remesh + perfil ``post_voxel``.
"""
...
def remesh_textured_glb(
    ...
    *,
    ...
    Args:
        ...
        repair: Aplica reparo pré/pós-decimação (default True). Use False
            para decimação crua (debug/inputs já limpos).
    ...
    # Hardlinks (painted ↔ lod0) partilham inode: escrever no path_out
    # corromperia todos os nomes. Unlink se nlink>1 → novo ficheiro.
