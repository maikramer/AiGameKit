...
- As etapas do pipeline (3D, rig, animate) são **auto-detetadas** a partir dos campos do manifest YAML (`generate_3d`, `generate_rig`, `generate_animate`) e dos blocos de perfil em `game.yaml` (`rigging3d`, `animator3d`). Usa **`--no-3d`**, **`--no-rig`**, **`--no-animate`** para desativar explicitamente uma etapa. Por exemplo, um bloco `rigging3d: {}` em `game.yaml` activa o rig para personagens mesmo sem `generate_rig=true` no manifest.
...
- **Multi-GPU:** `--gpu-ids 0,1` auto-deteta as GPUs disponíveis via `nvidia-smi` (ou aceita IDs explícitos separados por vírgula) e propaga `CUDA_VISIBLE_DEVICES` e `--gpu-ids` a todos os sub-tools (text2d, text3d, paint3d, …) — incluindo no payload vramd.
- **vramd:** o batch garante o supervisor, exporta `VRAMD_PRIORITY=batch`, herda outras `VRAMD_*` do ambiente, e o dashboard (se activo) mostra `vramd q=… run=… eta=…`. Opt-in ruidoso: `--vramd-stream` → `VRAMD_STREAM=1` nos filhos. O mesmo padrão de env aplica-se a `resume` e `dream`. **hw-auto** preenche o pico no payload (sem CLI `--low-vram` / `--memory-efficient`). **Waves** (`ums_batch.py`): shape (`text3d`) + paint (`paint3d`) + opcionais `text2d` / ícones (`text2d --category icon`) / `texture2d` / `skymap2d` / `text2sound` / `terrain3d`. Ops: [`docs/findings/UMS_VRAM_FINDINGS.md`](../docs/findings/UMS_VRAM_FINDINGS.md).
...
Exemplo mínimo (com linhas `generate_3d=true`):
...
| `text2d` | Bloco opcional: `cpu`, `width`, `height` |
...
Flags `low_vram`, `--low-vram`, `low_vram_mode` e CLI `--memory-efficient` foram **removidas**. VRAM = **vramd** + **hw-auto** (sinais de pico só no payload). Para assets sérios: `preset: balanced` ou `fast`; fecha apps que consumam VRAM.
...
| `RIGGING3D_BIN` | Caminho para `rigging3d` (ou `python -m rigging3d`) quando o rig está activo |
| `ANIMATOR3D_BIN` | Caminho para `animator3d` quando a animação está activa |
