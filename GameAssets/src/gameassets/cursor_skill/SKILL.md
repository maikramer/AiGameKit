...
- **`text2d`**: `cpu`, `width`, `height` (resolução 2D). A VRAM resolve-se pelo hw-auto (GPUs pequenas → offload).
...
**Atenção:** o GameAssets não propaga flags de VRAM; cada sub-tool (text2d, text3d, paint3d, …) usa hw-auto próprio. Para reduzir VRAM no Text3D, usar `preset: fast` ou baixar `steps`/`octree_resolution`/`num_chunks`.
