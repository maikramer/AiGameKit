"""vramd_batch: peak VRAM via hw_auto, sem hardcode sdnq-uint8."""
...
class TestShapeSpecsHwAuto:
    def test_payload_uses_hw_auto_not_hardcoded_uint8(self, tmp_path: Path) -> None:
        ...
        payload_mods = _make_pkg("text3d.vramd_payload", build_generate_request=mock_build)
...
class TestOptionalWaveOrFallback:
    ...
    def test_text2d_wave_mocked(self, tmp_path: Path) -> None:
        ...
        with (
            patch("gameassets.vramd_batch.text2d_specs_from_items", return_value=fake_specs),
            patch("gameassets.vramd_batch.run_gpu_wave", return_value=fake_results) as wave,
        ):
            out = run_text2d_wave_or_fallback(items, manifest_dir=tmp_path, no_vramd=False)
    ...
    def test_texture2d_fallback_sentinel(self, tmp_path: Path) -> None:
        items = [{"id": "a", "prompt": "p", "output": "o.png"}]
        with (
            patch("gameassets.vramd_batch.texture2d_specs_from_items", return_value=[MagicMock()]),
            patch("gameassets.vramd_batch.run_gpu_wave", return_value=FALLBACK_SUBPROCESS),
        ):
            assert run_texture2d_wave_or_fallback(items, manifest_dir=tmp_path, no_vramd=False) is None
    ...
    def test_terrain_wave_mocked(self, tmp_path: Path) -> None:
        ...
        with (
            patch("gameassets.vramd_batch.terrain3d_specs_from_items", return_value=fake_specs),
            patch("gameassets.vramd_batch.run_gpu_wave", return_value=fake_results),
        ):
            out = run_terrain3d_wave_or_fallback(items, manifest_dir=tmp_path, no_vramd=False)
            assert out is not None
