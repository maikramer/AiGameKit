...
| `delta.rs` | **Terreno vivo (Fase 3):** `DeltaGrid` (recortes densos f32 ancorados ao lattice da grid e recortados ao mundo; uma edição funde-se com o recorte do TOPO que toca — os cantos da união vêm da vista de baixo, nunca NaN; acima do teto de recortes funde o par adjacente de menor união; `revision`), `EditedBase` (grid+recortes como um `HeightField`), `TerrainEdit` (lower/raise/flatten/crater com falloff; raio clampado a 96 m, profundidade relativa > 64 m é pedido malformado — `is_well_formed`; o `Flatten` leva cota absoluta; clamp a `[0, max-height]`) e `TerrainEditQueue` (`EDITS_PER_FRAME`=4/frame, `EDIT_QUEUE_CAP`=256 pendentes; `applied`/`rejected`) |
