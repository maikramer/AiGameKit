"""Suite de cobertura GameDevLab — glb_meta, regras, diff, imagens, paths, perf, CLI."""
...
from gamedev_lab import perf_analyze
from gamedev_lab.cli import main
from gamedev_lab.compare_images import _to_float_rgb_a, metrics_mae_rmse_ssim
...
class TestPaths:
    def test_repo_root_from_file_location(self) -> None:
        ...
        assert (root / "GameDevLab").is_dir()
        assert (root / "Text3D").is_dir()

    def test_repo_root_env_override(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.setenv("GAMEDEV_ROOT", str(tmp_path))
        assert gamedev_repo_root() == tmp_path.resolve()
...
class TestPerfAnalyze:
    ...
    def test_list_runs_after_insert(self, tmp_path: Path) -> None:
        ...
        from gamedev_shared.perfstore.models import RunRecord
