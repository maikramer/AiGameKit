...
|--------------|--------------------|------------------|
| Tribal knowledge | “You have to pass flag X after kill Y” | Documented defaults; vramd owns GPU |
...
3. Use `vramd status` / `vramd queue` when the GPU is busy — never invent kill rituals.
...
- “Just use `--no-vramd` and kill the other process” as the recommended path.
...
- vramd / anti-patterns: [`Vramd/README.md`](../../Vramd/README.md)
