...
2. Route GPU work through **vramd**; never teach kill/race/pkill as the normal path.
...
- Make failure messages name the next command (`vramd queue`, `resume`, `doctor`).
...
## 2. vramd is the GPU control plane

**Do**

- Call `try_vramd_delegation` / `delegate_to_vramd` before in-process GPU prep (`prepare_gpu_exclusive` only after vramd fail / `--no-vramd`).
- Fill peak fields via hw-auto / `with_vramd_peak_opts` (`sdnq_preset` / `memory_efficient` on the payload — not public CLI knobs).
- Use `vramd status|queue|wait|cancel` in runbooks and agent tips (`UMS_DO_NOT_KILL_TIP`).
- Keep interactive priority above batch unless the product explicitly wants otherwise.

**Don’t**

- Recommend `--gpu-kill-others`, pkill, or `kill` while vramd holds work.
...
- Bypass vramd in docs “for speed” without labeling it an advanced escape hatch.
...
| Extend | Avoid |
|--------|--------|
| vramd backend + peak metadata | “User must unload X before Y” |
...
- Add/adjust tests that lock the contract (CLI smoke, batch wiring, vramd client flags).
...
| GPU contention | `vramd queue` / wait / cancel |
