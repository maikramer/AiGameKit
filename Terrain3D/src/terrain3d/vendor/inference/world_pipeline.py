...
from terrain3d.vendor.inference.portable_rng import fill_standard_normal, next_seed, standard_normal
import time
import json
import os
import atexit
...
from terrain3d.vendor.inference.synthetic_map import make_synthetic_map_factory
from terrain3d.vendor.data.laplacian_encoder import laplacian_decode, laplacian_denoise
from terrain3d.vendor.models.edm_unet import EDMUnet2D
from terrain3d.vendor.common.model_utils import resolve_model_path, MODEL_PATHS
import numpy as np
from typing import Union
from terrain3d.vendor.models.mp_layers import mp_concat
from terrain3d.vendor.scheduler.dpmsolver import EDMDPMSolverMultistepScheduler
import matplotlib.pyplot as plt
import skimage
from terrain3d.vendor.inference.postprocessing import *
...
class WorldPipeline(ConfigMixin):
    ...
    def __init__(
        ...
        *,
        T: int = 2,
        num_inference_steps: int = 20,  # AiGameKit: coarse diffusion timesteps (upstream hardcodes 20)
        log_mode: str = 'info',
    ...
    def change_seed(self, seed: int | None = None) -> bool:
        ...
        Args:
            seed: Explicit 64-bit seed (masked to 64 bits). If None, draws a new seed
                via :func:`terrain3d.vendor.inference.portable_rng.next_seed`.
    ...
    def set_custom_conditioning_import(
        ...
        internal units as :mod:`~terrain3d.vendor.inference.tiff_export` (e.g. temp std
        as °C×100).
    ...
    def _coarse_inference(self, ctx, scheduler, weight_window, t_cond, cond_inputs, pool_size=1):
        ...
        scheduler.set_timesteps(self.num_inference_steps)  # AiGameKit: era hardcoded 20 no upstream
