#!/usr/bin/env python3
"""CLI aigamekit-lab — debug, bancadas, profiling."""
...
from aigamekit_lab.cli_rich import click
from aigamekit_lab.compare_inspect import diff_inspect
from aigamekit_lab.debug_tools import inspect_glb
...
Exemplos:
  ...
  aigamekit-lab bench sdnq-sweep --mesh modelo.glb --image ref.png --target-vram-mb 5500
  aigamekit-lab profile cprofile -o out.prof meu_script.py -- --arg 1
  aigamekit-lab perf list --tool text2d --limit 10
  aigamekit-lab perf recommend text2d --vram 8000
  aigamekit-lab perf vram --tool text3d
"""


@click.group(epilog=EPILOG)
@click.version_option(__version__, prog_name="aigamekit-lab")
def main() -> None:
    """Ferramentas de laboratório AiGameKit."""
...
def debug_bundle(
    ...
    bundle: dict[str, Any] = {
        "tool": "aigamekit_lab.debug.bundle",
        "aigamekit_lab_version": __version__,
    ...
    if include_rig:
        from aigamekit_lab.renderer import render_screenshots as _rig_render
        from aigamekit_lab.renderer import render_weight_heatmap
...
def debug_cut_review(
    ...
    from aigamekit_lab.cut_review import review_cut
...
def debug_inspect_rig(
    ...
    from aigamekit_lab.renderer import render_screenshots, render_weight_heatmap
...
def debug_inspect_material(
    ...
    from aigamekit_lab.renderer import render_inspect_material
...
def debug_turntable(
    ...
    from aigamekit_lab.renderer import render_turntable
...
def bench_paint_vram_cmd(
    ...
    from aigamekit_lab.bench_paint_vram import run_paint_quantization_bench
...
def bench_sdnq_sweep_cmd(
    ...
    from aigamekit_lab.sdnq_optimizer import run_sdnq_sweep_cli
...
def bench_batch_cmd(
    ...
    from aigamekit_lab.bench_batch import run_batch_bench_cli
...
def perf_list_cmd(tool: str | None, limit: int, db_path: Path | None) -> None:
    ...
    from aigamekit_lab.perf_analyze import list_runs, print_runs_table
...
def perf_show_cmd(run_id: int, db_path: Path | None) -> None:
    ...
    from aigamekit_lab.perf_analyze import print_spans_detail
...
def perf_summary_cmd(tool: str | None, gpu: str | None, quant: str | None, days: int, db_path: Path | None) -> None:
    ...
    from aigamekit_lab.perf_analyze import print_summary
...
def perf_vram_cmd(tool: str | None, gpu: str | None, days: int, db_path: Path | None) -> None:
    ...
    from aigamekit_lab.perf_analyze import print_vram_analysis
...
def perf_recommend_cmd(
    ...
    from aigamekit_lab.perf_analyze import print_recommend
...
def mesh_inspect_cmd(mesh_path: Path, json_out: Path | None, verbose: bool) -> None:
    ...
    from aigamekit_lab.mesh_inspector import MeshInspector, print_qa_report
...
def mesh_render_views_cmd(
    ...
    from aigamekit_lab.mesh_inspector import MeshInspector
