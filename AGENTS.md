...
- **topology-fix rápido (engine arrays)**: `text3d topology-fix --engine auto|arrays|bpy` (default `auto`). Os passos de filtro do `topology_clean` (welds, slivers, long edges, debris, shells, boundary) correm vetorizados em `gamedev_shared.mesh_repair_arrays` (numpy/scipy, sem bpy) quando o mesh não tem UVs/weights/shape-keys/armature; os passos topológicos (fill/caps/`make_watertight`) ficam em bmesh. No engine arrays o morph-close corre **depois** do weld (solidify em mesh split criava paredes por-triângulo; VDB não via volume). `morphological_close` com voxel clampado à grelha (`MORPH_MAX_GRID_AXIS=800`) sobe `dist_eff`/`wall` para a escala do voxel e usa cadeia reduzida (1 remesh); o decimate-back usa `fast_simplification` (dep Text3D) com fallback DECIMATE. Números: dead_bush 89s→27s, longhouse 2.35M tris >16min(DNF)→3m21s, SSIM ≥0.997. Estudo completo: `docs/TOPOLOGY_FIX_GPU_STUDY.md`.
