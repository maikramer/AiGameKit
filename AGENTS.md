...
Roadmap: Phases 0–3 ✅ — XML + spawn; terrain (heightfield + features); Luau scripting (mlua sandboxed, `viber.*`/`viber.ui.*` API in `Viber/docs/LUA_API.md`); physics via **Rapier** (`bevy_rapier3d`, declarative `collider`/`rigidbody`) + `simple-rpg` ported (combat, quests, economy, menus, travel, save, skills, living world — 10 loops done, see `docs/findings/VIBER_SIMPLE_RPG_PORT.md`). The `EngineConfig` tags (`NavMesh`, `SpawnGate`, `ProjectileTemplate` + `viber.fire_projectile`, `AdaptiveQuality`, `PostFxDebugToggle`) have runtime consumers. Open items: GPU vegetation instancing. **VibeGame (TS/browser) remains the browser engine** — Viber is the native track; both share the GLB/KTX2/meshopt assets produced by the `gameassets` pipeline.
