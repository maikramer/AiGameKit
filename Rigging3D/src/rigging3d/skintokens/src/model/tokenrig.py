...
from transformers import AutoConfig, AutoModelForCausalLM, LogitsProcessor, LogitsProcessorList  # type: ignore

LLM_LOCAL_DIR = Path("models/Qwen3-0.6B")

from ..rig_package.info.asset import Asset
from ..tokenizer.parse import get_tokenizer
from ..tokenizer.spec import DetokenizeOutput, Tokenizer
...
from .spec import ModelInput, ModelSpec, TokenRigResult, VaeInput
...
class TokenRig(ModelSpec):
    def __init__(self, model_config, transform_config, tokenizer_config=None):
        ...
        super().__init__(
            model_config=model_config, transform_config=transform_config, tokenizer_config=tokenizer_config
        )
        ...
        assert isinstance(self.mesh_encoder, MAP_MESH_ENCODER.michelangelo) or isinstance(
            self.mesh_encoder, MAP_MESH_ENCODER.michelangelo_encoder
        )
        ...
        self.transformer = AutoModelForCausalLM.from_config(config=llm_config, attn_implementation="sdpa").to(
            torch.bfloat16
        )
    ...
    def make_start_tokens(self, **kwargs) -> list[list[int]]:
        skeleton_tokens = kwargs.get("skeleton_tokens")
    ...
    def generate(
        ...
        **kwargs,
        ...
        learned_mesh_cond = encode_mesh_cond(
            self.mesh_encoder, self.output_proj, self.tokens_skin_cond, {"vertices": vertices, "normals": normals}
        )

        device = cond.device
        start_tokens = torch.tensor(
            self.make_start_tokens(
                device=device,
    ...
    def _debug_export(
        ...
        if inputs_ids.dim() == 2:
            assert cond_latents.dim() == cond.dim() == 3, (
                f"Expected 3 dimensions, got {cond_latents.dim()}, {cond.dim()}"
            )
            cond = cond[id]
    ...
    def process_fn(self, batch: list[ModelInput]) -> list[dict]:
        ...
        for b in batch:
            if b.tokens is not None:
                skeleton_tokens = np.pad(
                    b.tokens, ((0, max_length - b.tokens.shape[0])), "constant", constant_values=self.tokenizer.pad
                )
                skeleton_mask = np.pad(
                    np.ones_like(b.tokens), ((0, max_length - b.tokens.shape[0])), "constant", constant_values=0
                )
            else:
...
def _check(x: Tensor, s, m=None):
    ...
    for i, (dim_actual, dim_expected) in enumerate(zip(x.shape, s)):
        if dim_expected is not None and dim_expected != -1:
            if m is None:
                assert dim_actual == dim_expected, (
                    f"Shape mismatch at dim {i}: expected {dim_expected}, got {dim_actual}"
                )
            else:
                assert dim_actual == dim_expected, (
                    f"Shape mismatch at dim {i}: expected {dim_expected}, got {dim_actual}. Message: {m}"
                )
...
def encode_mesh_cond(mesh_encoder, output_proj, tokens_skin_cond, batch: dict) -> Tensor:
    ...
    if len(vertices.shape) == 3:
        ...
        shape_embed, latents, token_num, pre_pc = mesh_encoder.encode_latents(
            pc=vertices.unsqueeze(0), feats=normals.unsqueeze(0)
        )  # type: ignore
...
def encode(
    ...
    B = vae_input.B
    J = vae_input.max_J
    _, cond_latents, codes, _ = vae.encode(
        vae_input=vae_input, num_tokens=tokens_per_skin, full=True, encode_repeat=encode_repeat
    )
...
def prepare_llm_tokens(
    ...
    inputs_ids = (
        torch.ones(
            (B, skeleton_tokens.shape[1] + skin_tokens.shape[1] + 1), dtype=torch.long, device=skeleton_tokens.device
        )
        * tokenizer.pad
    )
...
def decode_multi(
    ...
    for i in range(B):
        ...
        try:
            ...
            print(f"decode_multi: error while decoding skeleton: {e!s}")
            continue
        J = detokenize_output.joints.shape[0]
