...
from .spec import ModelInput, ModelSpec, VaeInput
...
try:
    ...
    def flash_attn_func(q, k, v, *args, **kwargs):
        # (B, seq, heads, head_dim) -> SDPA's (B, heads, seq, head_dim)
        qt = q.transpose(1, 2)
        kt = k.transpose(1, 2)
        vt = v.transpose(1, 2)
...
class SkinVAEModel(ModelSpec):
    ...
    def decode(
        self, z: Tensor, sampled_cond: Tensor, cond_tokens: Tensor, full: bool = False, encode_repeat: int = 4
    ) -> Tensor:
    ...
    def predict_step(self, batch: dict) -> dict:
        ...
        _skin_pred = self.decode(
            z=z,
            sampled_cond=vae_input.get_flatten_uniform_cond(),
            cond_tokens=cond_tokens[vae_input.get_flatten_indices()],
            full=True,
            encode_repeat=8,
        )
