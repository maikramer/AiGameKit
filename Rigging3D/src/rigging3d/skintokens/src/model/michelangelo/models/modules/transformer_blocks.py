...
def flash_attention(q, k, v):
    with torch.nn.attention.sdpa_kernel(
        [torch.nn.attention.SDPBackend.FLASH_ATTENTION, torch.nn.attention.SDPBackend.MATH]
    ):
        if use_flash3.is_use:
...
class QKVMultiheadCrossAttention(nn.Module):
    def __init__(
        self, *, device: torch.device, dtype: torch.dtype, heads: int, flash: bool = False, n_data: int | None = None
    ):
...
class ResidualCrossAttentionBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.mlp = MLP(
            device=device, dtype=dtype, width=width, hidden_width_scale=mlp_width_scale, init_scale=init_scale
        )
