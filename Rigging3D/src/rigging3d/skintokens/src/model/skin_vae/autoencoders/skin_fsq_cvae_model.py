...
from .autoencoder_kl_tripo2 import Tripo2Decoder, Tripo2Encoder
from .FSQ import FSQ
...
class SkinFSQCVAEModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder = Tripo2Decoder(
            in_channels=self.embedder.out_dim + cond_channels,
    ...
    def _sample_features(self, x: torch.Tensor, num_tokens: int = 128, seed: int | None = None):
        """
    ...
    def get_qkv(self, x: torch.Tensor, num_tokens: int = 128, seed: int | None = None, not_get_q: bool = False):
    ...
    def _decode(
        ...
        for i in range(0, num_points, num_chunks):
            queries = torch.cat(
                [self.embedder(positions[:, i : i + num_chunks, :]), features[:, i : i + num_chunks, :]], dim=-1
            )
