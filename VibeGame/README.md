...
With the [AiGameKit](https://github.com/maikramer/AiGameKit) repo checked out, **Bun** and **Node** on `PATH`:
...
| `<AudioSource>`       | Audio emitter (Howler)                    | `url`, `loop`, `volume`, `spatial`                                |
...
<Scene canvas="#game-canvas" sky="#87ceeb">
  ...
  <Terrain heightmap="/assets/terrain/heightmap.png" world-size="256"></Terrain>
...
The GLTF bridge (`vibegame/extras/gltf-bridge`) provides functions for loading GLB/GLTF assets produced by the AiGameKit pipeline (Text3D, Paint3D, Rigging3D, Animator3D). All loaders attach KTX2 texture support when available and apply default shadow flags (`castShadow` + `receiveShadow`).
...
VibeGame includes 52 plugins (35 registered by default, 17 opt-in) organized by category. See [`src/plugins/README.md`](src/plugins/README.md) for the full architecture and plugin creation template, and [`docs/PLUGINS.md`](docs/PLUGINS.md) for the complete list.
...
`follow-camera` (use `player-controller` / `<ThirdPersonCamera>` today), `fog`, `joints`, `lod`, `network`, `respawn`, `sprite`, `line`, `text`, `text-3d`, `scene-manifest` (asset manifests load via the GLTF bridge and `gameassets handoff` instead).
...
End-to-end demo of the AiGameKit monorepo workflow:
...
│   ├── plugins/           — Plugin implementations (52; 35 default, 17 opt-in; see docs/PLUGINS.md)
...
## GLB Handoff from AiGameKit Pipeline

Assets generated by the AiGameKit monorepo tools can be loaded directly into VibeGame:
...
| **Text2Sound**                      | WAV audio                   | `<AudioSource url="…">` or `playAudioEmitter()`            |
| **Terrain3D**                       | heightmap.png, terrain.json | `<Terrain heightmap="…">`                                  |
