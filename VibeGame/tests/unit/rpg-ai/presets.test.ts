...
interface StubState {
  ...
  // The chase FSM attaches a NavMeshAgent; the navmesh is never ready in these
  // unit tests (isNavMeshReady() === false), so the agent is a no-op stub.
...
describe('creature presets — YAML load + registry lookup', () => {
  it('slime.yaml parses into a MeleeAiPreset with expected tuning', async () => {
    ...
    expect(slime?.assets.modelUrl).toBe('/assets/meshes/characters/slime_lod0.glb');
  ...
  it('goblin.yaml parses into a MeleeAiPreset distinct from slime', async () => {
    ...
    expect(goblin?.assets.modelUrl).toBe('/assets/meshes/characters/goblin_lod0.glb');
  });
