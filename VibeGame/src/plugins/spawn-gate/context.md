...
- **Surface sample cache**: `getGroundHeight` is a five-probe Catmull-Rom cross over the LOD lattice, and carrying is the steady state, so the naive form paid that probe for every character on every fixed step — with ~100 creatures standing still it dominated the system's cost while re-deriving a constant. The sampled height is memoised per entity (per `State`, so worlds cannot cross-talk) and re-probed only when the character drifts more than 3 cm horizontally or `getGroundRevision(state)` changes. That revision is bumped by `fireHeightmapReloadCallbacks` and `fireGroundMutationCallbacks`, i.e. every heightmap (re)load and every sampler carve (pads, lakes, rivers, roads). `resetUnburySurfaceCache(state)` drops it for tests/teardown.
...
- **Cached height, 3 cm resolution**: while a carried character stays inside the 3 cm epsilon its pin uses the memoised surface height, so on a steep slope the pin can lag the true surface by `slope × 3 cm`. A carve that changes the ground without going through `fireGroundMutationCallbacks` / `fireHeightmapReloadCallbacks` is invisible to a stationary character until it moves.
