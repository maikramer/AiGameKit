...
from .visualize_mesh_web import generate_static_html_content, save_visualization_data

# Happy path is dict/keypoints3d → NPZ (no Autodesk FBX SDK).
try:
...
class T2MRuntime:
    def __init__(
        ...
        if self.disable_prompt_engineering:
            ...
            # Lazy: openai / prompt_rewrite only when rewriter is enabled.
    ...
    def test_dit_inference(self, duration: float = 2.0, seed: int = 42) -> bool:
        ...
        try:
            ...
            print("\t>>> DiT forward pass completed successfully!")
            return True
