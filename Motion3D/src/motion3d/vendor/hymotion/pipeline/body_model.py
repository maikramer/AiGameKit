...
def _default_wooden_mesh_path() -> str:
    """AiGameKit vendor path (no Gradio scripts/ tree required)."""
    here = os.path.dirname(os.path.abspath(__file__))
    packaged = os.path.normpath(os.path.join(here, "..", "assets", "dump_wooden"))
    if os.path.isdir(packaged):
        return packaged
    return "scripts/gradio/static/assets/dump_wooden"
...
class WoodenMesh(torch.nn.Module):
    ...
    def __init__(self, model_path: str | None = None):
        ...
        if model_path is None:
            model_path = _default_wooden_mesh_path()
