...
# AiGameKit default: pull encoders from HF Hub (or cache paths via env).
# Upstream Gradio used local ``ckpts/``; override with HY_MOTION_*_PATH.
USE_HF_MODELS = os.environ.get("USE_HF_MODELS", "1") == "1"

if USE_HF_MODELS:
    QWEN_PATH = os.environ.get("HY_MOTION_QWEN_PATH", "Qwen/Qwen3-8B")
    CLIP_PATH = os.environ.get("HY_MOTION_CLIP_PATH", "openai/clip-vit-large-patch14")
else:
    QWEN_PATH = os.environ.get("HY_MOTION_QWEN_PATH", "ckpts/Qwen3-8B")
    CLIP_PATH = os.environ.get("HY_MOTION_CLIP_PATH", "ckpts/clip-vit-large-patch14")
