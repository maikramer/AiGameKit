"""FlashVDM / Hierarchical volume decoding for Part3D ShapeVAE.

Ported from Text3D Hunyuan ShapeVAE (Tencent Hunyuan non-commercial license).
Used to accelerate / improve part mesh decode vs Space ``latent2mesh_2`` fast path.

Modes:
  - hierarchical: near-surface sparse octree (~near-lossless vs dense)
  - flashvdm: hierarchical + adaptive top-k KV (faster, slight detail tradeoff)
"""

from __future__ import annotations

# ruff: noqa: B007, B905, F841, RUF003, RUF013, RUF059, SIM108
...
# ---------------------------------------------------------------------------
# FlashVDM cross-attention processors (set on ShapeVAE geo_decoder)
# ---------------------------------------------------------------------------
...
class HierarchicalVolumeDecoding:
    ...
    def __call__(
        ...
        bounds: float | tuple[float, ...] | list[float] = 1.01,
        ...
        **kwargs,
        ...
        for start in tqdm(
            range(0, xyz_samples.shape[0], num_chunks), desc=f"Hierarchical Volume Decoding [r{resolutions[0] + 1}]"
        ):
        ...
        for octree_depth_now in resolutions[1:]:
            ...
            if next_points.shape[0] == 0:
                ...
                _logger.warn(
                    f"Hierarchical decode: no near-surface voxels at r{octree_depth_now + 1}; keeping coarser grid."
                )
                break
            ...
            next_points = next_points.to(torch.float32) * torch.tensor(
...
class FlashVDMVolumeDecoding:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        _logger.info(f"FlashVDMVolumeDecoding Resolution: {resolutions}")
        ...
        xyz_samples = (
            xyz_samples.view(
                mini_grid_num, mini_grid_size, mini_grid_num, mini_grid_size, mini_grid_num, mini_grid_size, 3
            )
        ...
        for start in tqdm(
            range(0, xyz_samples.shape[0], num_batchs), desc="FlashVDM Volume Decoding", disable=not enable_pbar
        ):
        ...
        for octree_depth_now in resolutions[1:]:
            ...
            if next_points.shape[0] == 0:
                _logger.warn(
                    f"FlashVDM decode: no near-surface voxels at r{octree_depth_now + 1}; keeping coarser grid."
                )
                break
            next_points = next_points * torch.tensor(resolution, dtype=torch.float32, device=device) + torch.tensor(
                bbox_min, dtype=torch.float32, device=device
            )
