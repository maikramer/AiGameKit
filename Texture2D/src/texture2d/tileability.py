...
def make_seamless(image: Path | Image.Image, feather_ratio: float = 0.008) -> Image.Image:
    """Corrige as costuras de uma textura para tiling perfeito (post-process determinístico).

    A LoRA seamless não garante bordas idênticas por construção — em teste com
    ``score_tileability`` várias texturas geradas com prompts corretos ainda
    falhavam (score 0.0-0.7). Este post-process força as bordas opostas a serem
    idênticas: funde a banda ``N`` amostrada por :func:`score_tileability`
    (esquerda+direita, topo+base) numa média única e escreve-a em ambos os
    lados, com um feather estreito (``feather_ratio``, ~0.8% da largura por
    omissão) para a transição não ficar visível. Uma banda de feather larga
    (>2%) produz uma faixa desfocada percetível — mantenha-a estreita.

    Args:
        image: Caminho ou ``PIL.Image`` de entrada.
        feather_ratio: Fração da largura/altura usada para suavizar a transição
            entre a banda de costura forçada e o resto da imagem.

    Returns:
        Nova imagem RGB, seamless por construção (score_tileability ≈ 1.0).
    """
    img = image if isinstance(image, Image.Image) else Image.open(image)
    arr = numpy.asarray(img.convert("RGB"), dtype=numpy.float32).copy()
    height, width = arr.shape[0], arr.shape[1]

    n = max(1, min(8, width // 64))
    fw = max(n + 1, int(width * feather_ratio))
    fh = max(n + 1, int(height * feather_ratio))

    # Horizontal: funde a banda N esquerda/direita e escreve o mesmo conteúdo
    # em ambas, depois esbate (feather) a transição para dentro da imagem.
    edge_h = (arr[:, :n, :] + arr[:, -n:, :]) * 0.5
    t = numpy.linspace(0.0, 1.0, fw, dtype=numpy.float32)[None, :, None]
    arr[:, :fw, :] = edge_h[:, :1, :] * (1 - t) + arr[:, :fw, :] * t
    arr[:, :n, :] = edge_h
    arr[:, -fw:, :] = edge_h[:, -1:, :] * (1 - t[:, ::-1, :]) + arr[:, -fw:, :] * t[:, ::-1, :]
    arr[:, -n:, :] = edge_h

    # Vertical: mesmo truque nas linhas, já sobre a imagem corrigida horizontalmente.
    edge_v = (arr[:n, :, :] + arr[-n:, :, :]) * 0.5
    ty = numpy.linspace(0.0, 1.0, fh, dtype=numpy.float32)[:, None, None]
    arr[:fh, :, :] = edge_v[:1, :, :] * (1 - ty) + arr[:fh, :, :] * ty
    arr[:n, :, :] = edge_v
    arr[-fh:, :, :] = edge_v[-1:, :, :] * (1 - ty[::-1, :, :]) + arr[-fh:, :, :] * ty[::-1, :, :]
    arr[-n:, :, :] = edge_v

    result = numpy.clip(arr, 0, 255).astype(numpy.uint8)
